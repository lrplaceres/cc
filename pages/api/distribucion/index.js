...
export default async function handler(req, res) {
  ...
  switch (req.method) {
    case "POST":
      return await adicionarDistribucion(req, res);
    default:
      return await obtenerTodosAsignaciones(req, res);
  }
}

const adicionarDistribucion = async (req, res) => {
  try {
    var { fecha } = req.body;
    var fecha2 = moment(fecha).utc().format("YYYY-MM-DD");
    fecha = fecha2;
    const { uid, combustible, cantidad, entidad, prioridad, observaciones } =
      req.body.asignacion;
    const [result] = await pool
      .promise()
      .query("INSERT INTO distribucion SET ?", {
        uid,
        combustible,
        cantidad,
        entidad,
