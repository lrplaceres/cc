...
/*parametros: anno, mes
 *busca los despachos agrupados por tipo de combustible
 */
const obtenerDespachosMes = async (req, res) => {
  try {
    ...
    const [result] = await pool
      .promise()
      .query(
        "SELECT SUM(d.cantidad) as cantidad, c.nombre FROM despacho d INNER JOIN combustible c ON d.combustible = c.uid WHERE YEAR(d.fecha) = ? AND MONTH(d.fecha) = ? GROUP BY d.combustible ORDER BY c.nombre ASC",
        [anno, mes]
      );
