...
/*parametros: anno, mes
 *busca las entidades con mayor cantidad de autorizos
 */
const obtenerTopAutorizos = async (req, res) => {
  try {
    ...
    const [result] = await pool
      .promise()
      .query(
        "SELECT e.nombre, COUNT(a.entidad) AS cantidad FROM autorizo a INNER JOIN entidad e ON a.entidad = e.uid WHERE YEAR(a.fecha) = ? AND MONTH(a.fecha) = ? GROUP BY a.entidad ORDER BY cantidad DESC LIMIT 10",
        [anno, mes]
      );
